...
// --- 1. 后端逻辑 (Backend Logic) ---

// A. 添加新奖励 (Add new reward)
...
// B. 切换激活状态 (Deactivate / Activate reward)
...
if (isset($_POST['edit_details'])) {
    ...
    // 动态构建 SQL，初始包含基础字段
    $sql = "UPDATE reward SET Reward_name = ?, Points_Required = ?, Description = ?";
    $params = [$name, $points, $desc];

    // 如果上传了新照片，则加入更新列表
    if (!empty($_FILES['edit_image']['name'])) {
        $targetDir = "uploads/";
        $fileName = time() . "_updated_" . basename($_FILES['edit_image']['name']);
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 sticky top-6">
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-4">
                    <div class="group relative w-full h-36 border-2 border-dashed border-gray-300 rounded-lg hover:bg-gray-50 hover:border-gray-400 transition flex flex-col items-center justify-center cursor-pointer overflow-hidden bg-white">
                        ...
                        <input type="file" name="image" accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer z-20" onchange="handleImageUpload(this)">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div><label class="text-xs font-bold text-gray-500 uppercase">Points</label><input type="number" name="points" required class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black outline-none text-sm"></div>
                        <div><label class="text-xs font-bold text-gray-500 uppercase">Stock</label><input type="number" name="stock" required class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black outline-none text-sm"></div>
                    </div>

                    <div><label class="text-xs font-bold text-gray-500 uppercase">Description</label><textarea name="desc" rows="2" class="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black outline-none text-sm"></textarea></div>

                    <button type="submit" name="add_reward" class="w-full bg-black hover:bg-gray-800 text-white font-bold py-3 rounded-lg shadow-lg transition transform hover:-translate-y-0.5 text-sm">Add to Inventory</button>
                </form>
            </div>
        </div>
                            ...
                            <td class="p-4 pl-6 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] px-1.5 py-0.5 rounded border <?php echo strtolower($r['Type'])=='physical' ? 'bg-orange-50 text-orange-600 border-orange-200' : 'bg-blue-50 text-blue-600 border-blue-200'; ?>"><?= htmlspecialchars($r['Type']) ?></span>
                                        <span class="text-xs text-gray-500">Stock: <b><?= $r['Stock'] ?></b></span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-4 text-center"><span class="bg-indigo-50 text-indigo-700 py-1 px-2 rounded text-xs font-bold border border-indigo-100"><?php echo $r['Points_Required']; ?> pts</span></td>
                            <td class="p-4 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold <?= $r['Status'] === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' ?>">
                                    <span class="w-1.5 h-1.5 <?= $r['Status'] === 'Active' ? 'bg-green-500' : 'bg-gray-400' ?> rounded-full mr-1.5"></span> <?= $r['Status'] ?>
                                </span>
                            </td>
                            <td class="p-4 pr-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick="openEditModal('<?= $r['Reward_ID'] ?>', '<?= htmlspecialchars($r['Reward_name'], ENT_QUOTES) ?>', '<?= $r['Points_Required'] ?>', '<?= htmlspecialchars($r['Description'], ENT_QUOTES) ?>')" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition"><i class="fa-solid fa-pen-to-square"></i></button>
                                    <button onclick="quickStock(<?= $r['Reward_ID'] ?>, <?= $r['Stock'] ?>)" class="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition"><i class="fa-solid fa-boxes-stacked"></i></button>
                                    <form method="POST" class="inline">
                                        <input type="hidden" name="toggle_status" value="1">
                                        <input type="hidden" name="reward_id" value="<?= $r['Reward_ID'] ?>">
                                        <input type="hidden" name="current_status" value="<?= $r['Status'] ?>">
                                        <button type="submit" class="p-2 text-gray-400 hover:<?= $r['Status'] === 'Active' ? 'text-red-600 hover:bg-red-50' : 'text-green-600 hover:bg-green-50' ?> rounded-lg transition"><i class="fa-solid <?= $r['Status'] === 'Active' ? 'fa-eye-slash' : 'fa-eye' ?>"></i></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
...
<script>
    ...
    function openEditModal(id, name, points, desc) {
        Swal.fire({
            ...
            width: '500px',
            html: `
                <form id="editForm" method="POST" enctype="multipart/form-data" class="text-left space-y-4 mt-4">
                    <input type="hidden" name="edit_details" value="1">
                    <input type="hidden" name="edit_id" value="${id}">
                    
                    <div>
                        <label class="text-xs font-bold text-gray-500 uppercase">Item Name</label>
                        <input name="edit_name" class="w-full mt-1 p-2 border border-gray-300 rounded text-sm outline-none focus:ring-2 focus:ring-black" value="${name}">
                    </div>
                    
                    <div>
                        <label class="text-xs font-bold text-gray-500 uppercase">Points Required</label>
                        <input type="number" name="edit_points" class="w-full mt-1 p-2 border border-gray-300 rounded text-sm outline-none focus:ring-2 focus:ring-black" value="${points}">
                    </div>
                    
                    <div>
                        <label class="text-xs font-bold text-gray-500 uppercase">Description</label>
                        <textarea name="edit_desc" class="w-full mt-1 p-2 border border-gray-300 rounded text-sm outline-none focus:ring-2 focus:ring-black" rows="3">${desc}</textarea>
                    </div>

                    <div>
                        <label class="text-xs font-bold text-gray-500 uppercase">Update Photo (Optional)</label>
                        <input type="file" name="edit_image" accept="image/*" class="w-full mt-1 p-2 border border-gray-300 rounded text-sm bg-gray-50 cursor-pointer">
                    </div>
                </form>`,
    ...
    new Chart(ctx, { type: 'bar', data: { labels: labels, datasets: [{ label: 'Current Stock', data: dataPoints, backgroundColor: '#10b981', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true } } } });
    ...
    function handleImageUpload(input) {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                placeholder.classList.add('bg-white/80', 'p-2', 'rounded');
            }
